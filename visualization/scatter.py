...
def scatter_plot(simulator):
    ...
    for drone1 in simulator.drones:
        for drone2 in simulator.drones:
            if drone1.identifier != drone2.identifier:
                if drone1.sensor_drone_flag==0:
                    ax.scatter(drone1.coords[0], drone1.coords[1], drone1.coords[2], c='green', s=30)
    ...
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    plt.savefig(f'visualization/figures_initialpos/positions.png', dpi=300, bbox_inches='tight')
    plt.show()
