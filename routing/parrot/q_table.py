...
class Qtable:
    ...
    def __init__(self, env, my_drone):
        ...
        self.q_table = np.zeros((config.NUMBER_OF_NODES, config.NUMBER_OF_NODES), dtype=object)
        # initialize the q-table
        for i in range(config.NUMBER_OF_NODES):
            for j in range(config.NUMBER_OF_NODES):
    ...
    def take_action(self, my_drone, dst_drone):
        ...
        q_list = [self.q_table[dst_drone_id, _][0] for _ in range(config.NUMBER_OF_NODES)]
