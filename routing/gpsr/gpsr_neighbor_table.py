...
class GpsrNeighborTable:
    ...
    def add_neighbor(self, hello_packet, cur_time):
        print(f"[NEIGHBOR] Adding neighbor {hello_packet.src_drone.identifier} to table of {self.my_drone.identifier}")
    ...
    def best_neighbor(self, my_drone, dst_drone):
        print(f"[GPSR] Routing from {my_drone.identifier} to {dst_drone.identifier}")
        print(f"[GPSR] My coords: {my_drone.coords}")
        print(f"[GPSR] Destination coords: {dst_drone.coords}")
        print(f"[GPSR] Neighbor Table of {my_drone.identifier}: {list(self.neighbor_table.keys())}")
        """
        ...
        if self.have_void_area == 1:
            ...
            for key in self.neighbor_table.keys():
                ...
                if angle_deg <= best_angle:
                    ...
                    best_id = key
        print(f"[GPSR] Selected next hop: {best_id}")
